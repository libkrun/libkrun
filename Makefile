...
.PHONY: install clean test test-prefix gen-libkrun-bindings verify-libkrun-bindings gen-init-blob-bindings $(LIBRARY_RELEASE_$(OS)) $(LIBRARY_DEBUG_$(OS)) libkrun.pc libkrun_init.pc clean-sysroot clean-all
...
verify-libkrun-bindings: gen-libkrun-bindings
	@git diff --exit-code \
		src/libkrun-via-cdylib-weak/ffier-krun.json \
		$(LIBRARY_HEADER) \
	|| { echo "error: libkrun bindings are out of date — run 'make gen-libkrun-bindings'"; exit 1; }
