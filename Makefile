...
feature_bool = $(if $(filter 1,$(1)),1,0)
ACTIVE_BUILD_TAG = blk$(call feature_bool,$(BLK))-net$(call feature_bool,$(NET))-gpu$(call feature_bool,$(GPU))-input$(call feature_bool,$(INPUT))-vhost$(call feature_bool,$(VHOST_USER))-timesync$(call feature_bool,$(TIMESYNC))-virgl$(call feature_bool,$(VIRGL_RESOURCE_MAP2))-initblob$(if $(filter 0,$(INIT_BLOB)),0,1)$(VARIANT)
ACTIVE_TARGET_DIR = target/build-$(ACTIVE_BUILD_TAG)
ACTIVE_RELEASE_DIR = $(ACTIVE_TARGET_DIR)/release
ACTIVE_DEBUG_DIR = $(ACTIVE_TARGET_DIR)/debug
FFI_HELPER_TARGET_DIR = target/ffi-helper
...
.PHONY: install clean test test-prefix gen-libkrun-bindings gen-init-blob-bindings verify-init-blob-bindings $(LIBRARY_RELEASE_$(OS)) $(LIBRARY_DEBUG_$(OS)) libkrun.pc libkrun_init.pc clean-sysroot clean-all
...
gen-libkrun-bindings:
	cargo build -p libkrun --features ffi,blk,net
	cp target/ffier-krun.json include/ffier-krun.json
	{ echo '/* @generated by: make gen-libkrun-bindings — do not edit manually. */'; \
	  cargo run -p libkrun --features ffi,blk,net --bin libkrun-gen -- c-header; } > $(LIBRARY_HEADER)
	{ printf '// @generated by: make gen-libkrun-bindings — do not edit manually.\n#![allow(\n    dead_code,\n    unused_unsafe,\n    clippy::missing_safety_doc,\n    clippy::needless_lifetimes,\n    clippy::new_without_default,\n    clippy::type_complexity\n)]\n'; \
	  cargo run -p libkrun --features ffi,blk,net --bin libkrun-gen -- rust-client --weak; } > src/libkrun-via-cdylib-weak/src/lib.rs
	{ printf '// @generated by: make gen-libkrun-bindings — do not edit manually.\n#![allow(\n    dead_code,\n    unused_unsafe,\n    clippy::missing_safety_doc,\n    clippy::needless_lifetimes,\n    clippy::new_without_default,\n    clippy::type_complexity\n)]\n'; \
	  cargo run -p libkrun --features ffi,blk,net --bin libkrun-gen -- rust-client; } > src/libkrun-via-cdylib/src/lib.rs
...
# Verify that checked-in generated files are up to date (for CI).
verify-init-blob-bindings: gen-init-blob-bindings
	@git diff --exit-code init/init-blob-via-cdylib/ffier-krun_init.json \
		$(LIBRARY_HEADER_INIT) \
	|| { echo "error: init-blob bindings are out of date — run 'make gen-init-blob-bindings'"; exit 1; }
...
$(LIBRARY_RELEASE_$(OS)): $(INIT_BINARY_BSD)
	CARGO_TARGET_DIR=$(ACTIVE_TARGET_DIR) cargo build --release $(FEATURE_FLAGS)
	CARGO_TARGET_DIR=$(FFI_HELPER_TARGET_DIR) cargo build --release -p libkrun --features ffi,blk,net
	CARGO_TARGET_DIR=$(ACTIVE_TARGET_DIR) cargo build --release -p krun-init-blob --features ffi
ifeq ($(SEV),1)
	mv $(ACTIVE_RELEASE_DIR)/libkrun.so $(ACTIVE_RELEASE_DIR)/$(KRUN_BASE_$(OS))
endif
ifeq ($(AWS_NITRO),1)
	mv $(ACTIVE_RELEASE_DIR)/libkrun.so $(ACTIVE_RELEASE_DIR)/$(KRUN_BASE_$(OS))
endif
...
ifeq ($(OS),Darwin)
	mv $(ACTIVE_RELEASE_DIR)/libkrun.dylib $(ACTIVE_RELEASE_DIR)/$(KRUN_BASE_$(OS))
endif
	mkdir -p target/release
	cp $(ACTIVE_RELEASE_DIR)/$(KRUN_BASE_$(OS)) $(LIBRARY_RELEASE_$(OS))
	cp $(ACTIVE_RELEASE_DIR)/$(KRUN_INIT_BASE_$(OS)) target/release/$(KRUN_INIT_BINARY_$(OS))

$(LIBRARY_DEBUG_$(OS)): $(INIT_BINARY_BSD)
	CARGO_TARGET_DIR=$(ACTIVE_TARGET_DIR) cargo build $(FEATURE_FLAGS)
	CARGO_TARGET_DIR=$(ACTIVE_TARGET_DIR) cargo build -p krun-init-blob --features ffi
ifeq ($(SEV),1)
	mv $(ACTIVE_DEBUG_DIR)/libkrun.so $(ACTIVE_DEBUG_DIR)/$(KRUN_BASE_$(OS))
endif
ifeq ($(TDX),1)
	mv $(ACTIVE_DEBUG_DIR)/libkrun.so $(ACTIVE_DEBUG_DIR)/$(KRUN_BASE_$(OS))
endif
	mkdir -p target/debug
	cp $(ACTIVE_DEBUG_DIR)/$(KRUN_BASE_$(OS)) $(LIBRARY_DEBUG_$(OS))
	cp $(ACTIVE_DEBUG_DIR)/$(KRUN_INIT_BASE_$(OS)) target/debug/$(KRUN_INIT_BINARY_$(OS))
